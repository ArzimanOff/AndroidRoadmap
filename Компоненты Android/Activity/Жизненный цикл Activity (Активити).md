...
<br> Но, если передается не `null` объект `savedInstanceState`, то это значит, что активность перезапускается, 
...
<br> Он вызывается, когда активность становится видимой для пользователя, 
но еще не готова к взаимодействию. 
На этом этапе активность уже загружена в память и отображает интерфейс,
...
которые должны быть активны, пока активность видима, 
...
* `onStart()` вызывается каждый раз, когда активность становится видимой, будь то при её первом запуске или при возвращении к активности после сворачивания приложения.
* Если пользователь минимизирует приложение, а затем возвращается, то активность снова вызовет `onStart()`.

**Реакция на действия пользователя:** \
Метод не подразумевает взаимодействие пользователя с приложением — оно еще не активно для ввода. 
Только в `onResume()` активность получает полный фокус для взаимодействия.

**Время выполнения фоновых задач:** \
Этот метод полезен для инициализации элементов, которые будут активны, пока активность видна. 
